...
void FT6236_Init(void)
{
	...
	GPIO_Init(GPIOC,&GPIO_InitStructure);					//初始化对应GPIOG	
	GPIO_SetBits(GPIOC,GPIO_Pin_0 | GPIO_Pin_2);			//PG.7.8 输出高	
	/******  PC3:片选RST  ***************************************/	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;	//PC3	
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;		//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		//IO口速度为50MHz	
	GPIO_Init(GPIOC,&GPIO_InitStructure);					//初始化对应GPIOC		
	GPIO_SetBits(GPIOC,GPIO_Pin_3);							//PC.7 输出高
	/**********************PC6，中断INT*********************/
	...
	NVIC_InitStructure.NVIC_IRQChannel = EXTI1_IRQn ;	//使能TOUCH_INT所在的外部中断通道
...
*/
void EXTI1_IRQHandler(void)		//中断名在这个文件里startup_stm32f10x_hd.s 
{
    ...
    EXTI_ClearITPendingBit(EXTI_Line1); //清除 LINE6 上的中断标志位
}
